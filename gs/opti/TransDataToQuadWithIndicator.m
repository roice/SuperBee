...
persistent countprint;
   ...
   sbsp_size = int8(12);
   sbsp_cmd = int8(61);
   sbsp_data = [int32(11132), int32(11057), int32(10000)];
   % generate sequence
   trans_seq(1) = int8('$');
   trans_seq(2) = int8('B');
   trans_seq(3) = int8('<');
   ...
   for index_crc = 1:1:size(sbsp_data,2)
        trans_seq((index_crc-1)*4 + 1 + 5) = int8(mod(sbsp_data(index_crc),256));
        trans_seq((index_crc-1)*4 + 2 + 5) = int8(mod(sbsp_data(index_crc)/256,256));
        trans_seq((index_crc-1)*4 + 3 + 5) = int8(mod(sbsp_data(index_crc)/256/256,256));
        trans_seq((index_crc-1)*4 + 4 + 5) = int8(mod(sbsp_data(index_crc)/256/256/256,256));
   end
   % calculate CRC
